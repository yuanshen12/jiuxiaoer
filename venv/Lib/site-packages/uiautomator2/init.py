...
def app_uiautomator_apk_urls():
    ret = []
    for name in ["app-uiautomator.appmall", "app-uiautomator-test.appmall"]:
...
class Initer():
    ...
    def is_apk_outdated(self):
        """
        If appmall signature mismatch, the uiautomator test will fail to start
        ...
        self.logger.debug("appmall-debug package-info: %s", apk_debug)
        self.logger.debug("appmall-debug-test package-info: %s", apk_debug_test)
    ...
    def _install_uiautomator_apks(self):
        ...
        for filename, url in app_uiautomator_apk_urls():
            ...
            package_name = "com.github.uiautomator.test" if "test.appmall" in url else "com.github.uiautomator"
            if os.getenv("TMQ"):

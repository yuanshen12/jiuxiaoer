...
class Switch_foreign(Common):
    ...
    place = (By.XPATH, "//*[@id='55']/aside[2]/div[1]")
    ethyl = (By.XPATH, "//*[@id='56']/aside[2]/div[1]")
    capacity = (By.XPATH, "//*[@id='57']/aside[2]/div[2]")
    suer = (By.XPATH, "//*[contains(text(), '确定')]")
    reset = (By.XPATH, "//*[contains(text(), '重置')]")
    # 葡萄酒
    foreigns = (By.XPATH, "//*[contains(text(), '洋酒')]")
...
# 从首页获取预期值（洋酒）
    def Foreign(self):
        self.wait_time(2)
        try:
            grape_text = self.driver.find_element(*self.foreigns)
        except:
            return False
        else:
            grape_text.click()
...
# 筛选（产地）
    def Place(self):
        ...
        try:
            self.driver.find_element(*self.screen).click()
            self.driver.swipe(int(1048)/2,int(3500)/2,int(1048)/2,int(2000)/4,duration=sleep(1))
            self.wait_time(3)
...
# 筛选（酒精度）
    def Ethyl(self):
        ...
        try:
            self.driver.find_element(*self.screen).click()
            self.driver.swipe(int(1048)/2,int(3500)/2,int(1048)/2,int(100)/4,duration=sleep(1))
            self.wait_time(2)

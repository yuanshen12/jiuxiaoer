...
header "ExcelFormulaParser.__init__" {
    cls.rpn = ""
    cls.sheet_references = []
    cls.xcall_references = []
}
...
expr[arg_type]
    ...
    prec0_expr[arg_type]
        (
            ...
            prec0_expr[arg_type] { cls.rpn += op }
        )*
    ;
...
prec0_expr[arg_type]
    : prec1_expr[arg_type]
        (
            ...
            prec1_expr[arg_type] { cls.rpn += op }
        )*
    ;
...
prec2_expr[arg_type]
    : prec3_expr[arg_type]
        (
            ...
            prec3_expr[arg_type] { cls.rpn += op }
        )*
    ;
...
prec3_expr[arg_type]
    : prec4_expr[arg_type]
        (
            ...
            prec4_expr[arg_type] { cls.rpn += op }
        )*
    ;
...
prec4_expr[arg_type]
    : prec5_expr[arg_type]
        (
            PERCENT { cls.rpn += struct.pack('B', ptgPercent) }
        )?
    ;

prec5_expr[arg_type]
    : primary[arg_type]
    | SUB primary[arg_type] { cls.rpn += struct.pack('B', ptgUminus) }
    ;

primary[arg_type]
    : TRUE_CONST
        {
            cls.rpn += struct.pack("2B", ptgBool, 1)
        }
    | FALSE_CONST
        {
            cls.rpn += struct.pack("2B", ptgBool, 0)
        }
    | str_tok:STR_CONST
        {
            cls.rpn += struct.pack("B", ptgStr) + upack1(str_tok.text[1:-1].replace("\"\"", "\""))
        }
    | int_tok:INT_CONST
        {
            ...
            if int_value <= 65535:
                cls.rpn += struct.pack("<BH", ptgInt, int_value)
            else:
                cls.rpn += struct.pack("<Bd", ptgNum, float(int_value))
        }
    | num_tok:NUM_CONST
        {
            cls.rpn += struct.pack("<Bd", ptgNum, float(num_tok.text))
        }
    | ref2d_tok:REF2D
        {
            ...
            cls.rpn += struct.pack("<B2H", ptg, r, c)
        }
    | ref2d1_tok:REF2D COLON ref2d2_tok:REF2D
        {
            ...
            cls.rpn += struct.pack("<B4H", ptg, r1, r2, c1, c2)
        }
    | sheet1 = sheet
        { 
            sheet2 = sheet1
        ...
        {
            cls.rpn += struct.pack("<B", ptg)
            cls.sheet_references.append((sheet1, sheet2, len(cls.rpn)))
            cls.rpn += rpn_ref2d
        }
    | FUNC_IF
        LP expr["V"] (SEMICOLON | COMMA)
        {
            cls.rpn += struct.pack("<BBH", ptgAttr, 0x02, 0) // tAttrIf
            pos0 = len(cls.rpn) - 2
        }
        expr[arg_type] (SEMICOLON | COMMA)
        {
            cls.rpn += struct.pack("<BBH", ptgAttr, 0x08, 0) // tAttrSkip
            pos1 = len(cls.rpn) - 2
            cls.rpn = cls.rpn[:pos0] + struct.pack("<H", pos1-pos0) + cls.rpn[pos0+2:]
        }
        expr[arg_type] RP
        {
            cls.rpn += struct.pack("<BBH", ptgAttr, 0x08, 3) // tAttrSkip
            cls.rpn += struct.pack("<BBH", ptgFuncVarR, 3, 1) // 3 = nargs, 1 = IF func
            pos2 = len(cls.rpn)
            cls.rpn = cls.rpn[:pos1] + struct.pack("<H", pos2-(pos1+2)-1) + cls.rpn[pos1+2:]
        }
    | FUNC_CHOOSE
        {
            arg_type = "R"
        ...
        {
            rpn_start = len(cls.rpn)
            ref_markers = [len(cls.sheet_references)]
        }
        (
            (SEMICOLON | COMMA)
                { mark = len(cls.rpn) }
                (
                  expr[arg_type]
                | { cls.rpn += struct.pack("B", ptgMissArg) }
                )
                {
                    rpn_chunks.append(cls.rpn[mark:])
                    ref_markers.append(len(cls.sheet_references))
                }
        )*
        RP
        {
            cls.rpn = cls.rpn[:rpn_start]
            nc = len(rpn_chunks)
            ...
            for ic in xrange(nc):
                for refx in xrange(ref_markers[ic], ref_markers[ic+1]):
                    ref = cls.sheet_references[refx]
                    cls.sheet_references[refx] = (ref[0], ref[1], ref[2] + chunk_shift)
    ...
    | func_tok:NAME
        {
            ...
            if xcall:
                ...
                cls.xcall_references.append((func_toku, len(cls.rpn) + 1))
                cls.rpn += struct.pack("<BHHH",
                    ptgNameXR,
        ...
        LP arg_count = expr_list[arg_type_list, min_argc, max_argc] RP
        {
            ...
            if xcall:
                ...
                cls.rpn += struct.pack("<2BH", func_ptg, arg_count + 1, 255) // 255 is magic XCALL function
            elif min_argc == max_argc:
                func_ptg = ptgFuncR + _RVAdelta[func_type]
                cls.rpn += struct.pack("<BH", func_ptg, opcode)
            elif arg_count == 1 and func_tok.text.upper() == "SUM":
                cls.rpn += struct.pack("<BBH", ptgAttr, 0x10, 0) // tAttrSum
            else:
                func_ptg = ptgFuncVarR + _RVAdelta[func_type]
                cls.rpn += struct.pack("<2BH", func_ptg, arg_count, opcode)
        }
    | LP expr[arg_type] RP
        {
            cls.rpn += struct.pack("B", ptgParen)
        }
    ;

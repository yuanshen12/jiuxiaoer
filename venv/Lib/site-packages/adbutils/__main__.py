...
Commands:
    install     Install appmall to device
"""
...
class ReadProgress():
    def __init__(self, r, total_size: int, source_path=None):
        ...
        self._chunk_size = 0
        self._hash = ''
        self._tmpfd = None if source_path else tempfile.NamedTemporaryFile(suffix=".appmall")
...
def main():
    ...
    parser.add_argument("-i",
                        "--install",
                        help="install from local appmall or url")
    parser.add_argument(
        "--install-confirm",
        action="store_true",
        help="auto confirm when install (based on uiautomator2)")
    parser.add_argument("-u", "--uninstall", help="uninstall appmall")
    ...
    if args.install:
        ...
        dst = "/data/local/tmp/tmp-%d.appmall" % (int(time.time() * 1000))
        print("push to %s" % dst)
        
        start = time.time()
        d.sync.push(r, dst)
        
        # parse appmall package-name
        ...
        new_dst = "/data/local/tmp/tmp-%s.appmall" % r._hash[:8]
        d.shell(["mv", dst, new_dst])
        dst = new_dst
        info = d.sync.stat(dst)
        print("verify pushed appmall, md5: %s, size: %s" %
        ...
        try:
            ...
            if e.reason in ["INSTALL_FAILED_PERMISSION_MODEL_DOWNGRADE",
                ...
                sys.exit(
                    "Failure " + e.reason + "\n" +
                    "Remote appmall is not removed. Manually install command:\n\t"
